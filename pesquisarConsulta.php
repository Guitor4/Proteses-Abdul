...
if ($pesquisa != null) {
  header('location: pesquisarConsulta.php?pagina=1&search=' . $pesquisa);
}
...
//condições sql
$condicoes = [
  strlen($search) ? 'nomePaciente LIKE "%' . str_replace('', '%', $search) . '%" or prontuario LIKE "%' . str_replace('', '%', $search) . '%" or dataConsulta LIKE "%' . str_replace('', '%', $search) . '%" ' : null
