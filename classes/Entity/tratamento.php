    ...
    public static function getTratamentosInner($where,$order=null,$limit=null) {
       ...
       strlen($limit) ? $limit = ' LIMIT ' . $limit : '';
        ...
        return $db = (new db)->executeSQL('SELECT * FROM rastreio '
                ...
                . 'where idProtese not in (select fkProtese from rastreio where statusRastreio = "Finished") and idProtese not in (select idProtese from protese where status = "In third-party treatment" or status = "Delivered") '. $where.$order.$limit)
                ->fetchAll(PDO::FETCH_CLASS, self::class);
    }
